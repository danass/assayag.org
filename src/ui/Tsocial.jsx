    ...
    return (
        <>
            { data.length > 0? data.filter((tweet) => {
                if(!Meteor.user()) { return tweet.visible == true}
                    return true
            }).sort((a, b)=> {return new Date(b.pubDate._text) - new Date(a.pubDate._text)}).map((tweet, i) => {return <Box tweet={tweet} key={tweet.id}/>}): <Loading />}
